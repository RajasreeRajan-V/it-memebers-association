{{-- resources/views/mentor/mock-interviews/index.blade.php --}}
@extends('layouts.app')
...
<div class="rr-page">
    <div class="container-fluid px-3 px-md-4 py-4">
        ...
        <section class="rr-hero mb-4">
            ...
            <div class="rr-hero-content">
                ...
                <h1 class="rr-hero-title">
                    Manage Your
                    <br>
                    <span class="blue">Mock Interviews</span>
                </h1>

                <p class="rr-hero-description">
                    Review requests from students, track upcoming sessions and keep on top
                    of completed and cancelled interviews in one place.
                </p>
                ...
                <div class="rr-hero-stats">
                    ...
                    <div class="rr-mini-stat">
                        ...
                        <div>
                            <p class="rr-mini-value">{{ $stats['total'] ?? $interviews->total() }}</p>
                            <p class="rr-mini-label">Total Requests</p>
                        </div>
                    </div>
                    ...
                    <div class="rr-mini-stat orange">
                        ...
                        <div>
                            <p class="rr-mini-value">{{ $stats['pending'] ?? 0 }}</p>
                            <p class="rr-mini-label">Pending</p>
                        </div>
                    </div>
                    ...
                    <div class="rr-mini-stat green">
                        ...
                        <div>
                            <p class="rr-mini-value">{{ $stats['scheduled'] ?? 0 }}</p>
                            <p class="rr-mini-label">Scheduled</p>
                        </div>
                    </div>
            ...
            {{-- Decorative illustration: a video-call interview screen with a candidate --}}
        ...
        @php
            $tabsList = [
                ['label' => 'All',       'status' => '',          'count' => $stats['total'] ?? $interviews->total()],
                ['label' => 'Pending',   'status' => 'pending',   'count' => $stats['pending'] ?? 0],
                ['label' => 'Scheduled', 'status' => 'scheduled', 'count' => $stats['scheduled'] ?? 0],
                ['label' => 'Completed', 'status' => 'completed', 'count' => $stats['completed'] ?? 0],
                ['label' => 'Cancelled', 'status' => 'cancelled', 'count' => $stats['cancelled'] ?? 0],
            ];
            $tabBadge = [
                ''          => 'rr-badge-purple',
                'pending'   => 'rr-badge-orange',
                'scheduled' => 'rr-badge-blue',
        ...
        <div class="rr-tabbar mb-4">
            @foreach ($tabsList as $t)
                <a href="{{ route('mentor.mock-interviews.index', array_filter(['status' => $t['status'] ?: null])) }}"
                   class="rr-tab {{ request('status', '') === $t['status'] ? 'rr-tab-active' : '' }}">
                    {{ $t['label'] }}
        ...
        <div class="rr-row tr-row">
            ...
            {{-- ---- LEFT / MAIN COLUMN: Interview list ---- --}}
            <div class="rr-col-left tr-col-main">
                ...
                <div class="card rr-card border-0 mb-4">
                    <div class="card-header bg-white border-0 pb-3 pt-3 px-3">
                        <h2 class="h6 fw-semibold mb-0">Mock Interview Requests</h2>
                    </div>
                    ...
                    <div class="list-group list-group-flush">
                        @php
                            ...
                            $statusMap = [
                                'pending'   => ['label' => 'Pending',   'class' => 'rr-badge-orange'],
                                'scheduled' => ['label' => 'Scheduled', 'class' => 'rr-badge-blue'],
                                'completed' => ['label' => 'Completed', 'class' => 'rr-badge-green'],
                                'cancelled' => ['label' => 'Cancelled', 'class' => 'rr-badge-red'],
                            ];
                        @endphp
                        ...
                        @forelse ($interviews as $index => $interview)
                            @php
                                ...
                                $statusInfo = $statusMap[$interview->status] ?? ['label' => ucfirst($interview->status), 'class' => 'rr-badge-blue'];
                                $dateLabel = $interview->scheduled_at?->format('d M Y, h:i A')
                            ...
                            <div class="list-group-item py-3 border-0 border-bottom tr-training-item" role="button" tabindex="0"
                                 onclick="window.location='{{ route('mentor.mock-interviews.show', $interview) }}'"
                                 onkeydown="if(event.key==='Enter'){window.location='{{ route('mentor.mock-interviews.show', $interview) }}'}">
                                ...
                                <div class="d-flex align-items-start gap-3">
                                    ...
                                    <div class="flex-grow-1 min-w-0">
                                        <div class="d-flex justify-content-between align-items-start gap-2">
                                            <p class="fw-semibold mb-0 text-truncate">{{ $interview->student->name }}</p>
                                        </div>
                                        ...
                                        <div class="d-flex flex-wrap gap-2 align-items-center">
                                            ...
                                            @if ($dateLabel)
                                                <span class="small text-muted">
                                                    <i class="fa-regular fa-clock me-1"></i>{{ $dateLabel }}
                                                </span>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    <div class="row-actions-wrap flex-shrink-0" onclick="event.stopPropagation()">
                                        <a href="{{ route('mentor.mock-interviews.show', $interview) }}" class="row-actions-btn" aria-label="View">
                        ...
                        @empty
                            <div class="text-center text-muted py-5">
                                No mock interview requests found.
                            </div>
                        @endforelse
                    </div>
            ...
            <div class="tr-col-side">
                ...
                <div class="card rr-card rr-side-card border-0 mb-4">
                    ...
                    <div class="card-body">
                        <p class="rr-side-label mb-3"><i class="fa-solid fa-bolt"></i> Quick Filters</p>

                        <a href="{{ route('mentor.mock-interviews.index', ['status' => 'pending']) }}" class="tr-quick-action">
                            <span class="tr-qa-icon rr-tag-0"><i class="fa-regular fa-hourglass-half"></i></span>
                            <span class="tr-qa-text">
                                Pending Requests
                                <small>Respond and schedule new requests</small>
                            </span>
                        </a>
                        <a href="{{ route('mentor.mock-interviews.index', ['status' => 'scheduled']) }}" class="tr-quick-action">
                            <span class="tr-qa-icon rr-tag-1"><i class="fa-regular fa-calendar-check"></i></span>
                            <span class="tr-qa-text">
                                Upcoming Sessions
                                <small>See what's coming up next</small>
                            </span>
                        </a>
                    </div>
                </div>
                ...
                <div class="card rr-card rr-side-card border-0 mb-4">
                    <div class="rr-side-card-accent"></div>
                    <div class="card-body">
                        <p class="rr-side-label mb-3"><i class="fa-solid fa-chart-simple"></i> Interview Overview</p>
                        <ul class="list-unstyled small mb-0 rr-side-list tr-overview-list">
                            <li><span class="tr-dot" style="background:#3376F2;"></span>Total Requests <span class="tr-count">{{ $stats['total'] ?? $interviews->total() }}</span></li>
                            <li><span class="tr-dot" style="background:#F5A623;"></span>Pending <span class="tr-count">{{ $stats['pending'] ?? 0 }}</span></li>
                            <li><span class="tr-dot" style="background:#3376F2;"></span>Scheduled <span class="tr-count">{{ $stats['scheduled'] ?? 0 }}</span></li>
                            <li><span class="tr-dot" style="background:#22B573;"></span>Completed <span class="tr-count">{{ $stats['completed'] ?? 0 }}</span></li>
                            <li><span class="tr-dot" style="background:#EF5350;"></span>Cancelled <span class="tr-count">{{ $stats['cancelled'] ?? 0 }}</span></li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="card rr-card rr-side-card border-0">
                    <div class="rr-side-card-accent"></div>
                    <div class="card-body">
                        <p class="rr-side-label mb-3"><i class="fa-regular fa-lightbulb"></i> Tips for Great Mock Interviews</p>
                        <ul class="tr-tips-list small text-muted mb-0">
                            <li>Respond to pending requests promptly</li>
                            <li>Confirm topic and format before scheduling</li>
                            <li>Send a reminder ahead of each session</li>
                            <li>Give actionable feedback after each interview</li>
                            <li>Mark sessions completed to keep records tidy</li>
                        </ul>
                    </div>
                </div>
...
/* =========================================================
   ...
   Same design language as the Webinars & Workshops and
   Trainings pages: soft blue/purple palette, card panels,
   CSS-drawn hero illustration.
   ========================================================= */
...
/* =========================================================
   HERO VISUAL - a video-call interview screen with a candidate
   ========================================================= */
...
.row-actions-btn { width: 30px; height: 30px; border: none; background: #F3F4F6; border-radius: 8px; color: #6B7280; display: inline-flex; align-items: center; justify-content: center; cursor: pointer; text-decoration: none; }
