...
<div class="wz-shell">
    ...
    <div class="wz-header">
        <div>
            <h1>Edit {{ ucfirst($webinar->type) }}</h1>
            <p>Update the details below. Editing will resubmit this item for admin approval.</p>
        </div>
    ...
    @if ($webinar->status === 'rejected' && $webinar->admin_remarks)
    <div class="wz-remarks-box">
        <i class="fa-solid fa-triangle-exclamation"></i>
        <div>
            <strong>Admin remarks</strong>
            <p>{{ $webinar->admin_remarks }}</p>
        </div>
    </div>
    @endif
    ...
    <div class="wz-layout">
            ...
            <form method="POST" action="{{ route('mentor.webinars.update', $webinar) }}" enctype="multipart/form-data"
                ...
                <div class="wz-step-panel" data-panel="1">
                    ...
                    <div class="wz-field">
                        ...
                        <div class="wz-type-toggle">
                            <input type="radio" name="type" id="type_webinar" value="webinar" hidden
                                {{ old('type', $webinar->type) === 'webinar' ? 'checked' : '' }}>
                            ...
                            <input type="radio" name="type" id="type_workshop" value="workshop" hidden
                                {{ old('type', $webinar->type) === 'workshop' ? 'checked' : '' }}>
                    ...
                    <div class="wz-field">
                        ...
                        <input type="text" name="title" id="wzTitle" maxlength="100"
                            value="{{ old('title', $webinar->title) }}" placeholder="e.g. Mastering Laravel REST APIs"
                            required>
                    ...
                    <div class="wz-row">
                        <div class="wz-field">
                            ...
                            <select name="category" required>
                                <option value="" disabled {{ old('category', $webinar->category) ? '' : 'selected' }}>
                                ...
                                <option value="{{ $cat }}"
                                    {{ old('category', $webinar->category) === $cat ? 'selected' : '' }}>{{ $cat }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="wz-field">
                            ...
                            <select name="platform" required>
                                <option value="" disabled {{ old('platform', $webinar->platform) ? '' : 'selected' }}>
                                    Select platform</option>
                                @foreach (['Zoom', 'Google Meet', 'In-Person',] as $plat)
                                <option value="{{ $plat }}"
                                    {{ old('platform', $webinar->platform) === $plat ? 'selected' : '' }}>{{ $plat }}
                                </option>
                                @endforeach
                            </select>
                    ...
                    <div class="wz-field">
                        ...
                        <textarea name="description" id="wzDesc" maxlength="1000" rows="4"
                            ...
                            required>{{ old('description', $webinar->description) }}</textarea>
                ...
                <div class="wz-step-panel" data-panel="2" style="display:none;">
                    ...
                    <div class="wz-row">
                        <div class="wz-field">
                            ...
                            <input type="date" name="scheduled_date"
                                value="{{ old('scheduled_date', $webinar->scheduled_date?->format('Y-m-d')) }}"
                                required>
                        ...
                        <div class="wz-field">
                            ...
                            <input type="time" name="scheduled_time"
                                value="{{ old('scheduled_time', $webinar->scheduled_time) }}" required>
                    ...
                    <div class="wz-row">
                        ...
                        <div class="wz-field">
                            ...
                            <input type="number" min="1" name="capacity"
                                value="{{ old('capacity', $webinar->capacity) }}" placeholder="e.g. 100">
                    ...
                    <div class="wz-field">
                        ...
                        <input type="text" name="meeting_link" value="{{ old('meeting_link', $webinar->meeting_link) }}"
                ...
                <div class="wz-step-panel" data-panel="3" style="display:none;">
                    ...
                    <div class="wz-field">
                        ...
                        <textarea name="learning_outcomes_raw" id="wzOutcomes" rows="4"
                            placeholder="e.g.&#10;Build a REST API with Laravel&#10;Understand authentication with Sanctum&#10;Deploy to production">{{ old('learning_outcomes_raw', implode("\n", $webinar->learning_outcomes ?? [])) }}</textarea>
                    ...
                    <div class="wz-field">
                        ...
                        <textarea name="hands_on_activities" id="wzActivities" rows="3"
                            placeholder="e.g. Live coding exercise, Q&A workshop, group project">{{ old('hands_on_activities', $webinar->hands_on_activities) }}</textarea>
                    ...
                    <div class="wz-field">
                        ...
                        <textarea name="materials_required" id="wzMaterials" rows="3"
                            placeholder="e.g. Laptop with VS Code installed, GitHub account">{{ old('materials_required', $webinar->materials_required) }}</textarea>
                ...
                <div class="wz-step-panel" data-panel="4" style="display:none;">
                    ...
                    <div class="wz-tip-box">
                        <i class="fa-regular fa-lightbulb"></i>
                        <span>Listings with a banner image get noticeably more clicks than those without one. Leave this
                            untouched to keep your current banner.</span>
                    </div>

                    <div class="wz-field">
                        <label>Banner / Thumbnail <span class="opt">(optional)</span></label>

                        <div class="wz-dropzone" id="wzDropzone" style="{{ $webinar->banner ? 'display:none;' : '' }}">
                        ...
                        <div id="wzBannerPreviewWrap"
                            style="{{ $webinar->banner ? 'display:block;' : 'display:none;' }}">
                            <img id="wzBannerPreview" alt="Banner preview"
                                src="{{ $webinar->banner ? Storage::url($webinar->banner) : '' }}">
                            <button type="button" id="wzBannerChange"><i class="fa-solid fa-arrows-rotate"></i> Change
                                Image</button>
                        </div>

                        <div class="wz-notice" style="margin-top:12px;">
                            Leave empty to keep the current banner. Uploading a new image will replace it.
                        </div>
                ...
                <div class="wz-nav">
                    ...
                    <div class="wz-nav-right">
                        ...
                        <button type="submit" id="wzSubmitBtn" class="wz-btn-primary" style="display:none;">
                            <i class="fa-solid fa-arrows-rotate"></i> Update &amp; Resubmit
                        </button>
                    </div>
                </div>
            </form>
        ...
        <div class="wz-sidebar">
            ...
            <div class="wz-notice">
                <strong>Fields marked <span class="req">*</span> are required.</strong> Editing resubmits this item for
                admin approval.
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    (function initDropzone() {
        ...
        // "Change Image" re-opens the file picker to select a replacement.
    ...
    // ===================================================================
    // CHECKLIST (mini + sidebar) — pre-seeded from existing webinar data
    // ===================================================================
    ...
    function updateChecklist() {
        ...
        var bannerInput = document.getElementById('wzBannerInput');
        var hasExistingBanner = document.getElementById('wzBannerPreview') &&
            document.getElementById('wzBannerPreview').getAttribute('src');
