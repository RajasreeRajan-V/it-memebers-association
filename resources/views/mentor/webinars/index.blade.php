{{-- resources/views/mentor/webinars/index.blade.php --}}
@extends('layouts.app')
    ...
    <div class="container-fluid px-3 px-md-4 py-4">
        ...
        <section class="rr-hero mb-4">
            ...
            <div class="rr-hero-content">
                ...
                <h1 class="rr-hero-title">
                    Learn Through My
                    <br>
                    <span class="blue">Webinars &amp; Workshops </span>
                    
                </h1>

                <p class="rr-hero-description">
                    Create, manage and share your webinars and workshops. Track registrations,
                    approval status and performance in one place.
                </p>

                <div class="rr-hero-actions">
                    <a href="{{ route('mentor.webinars.create') }}" class="rr-hero-btn">
                ...
                <div class="rr-hero-stats">
                    ...
                    <div class="rr-mini-stat">
                        ...
                        <div>
                            ...
                            <p class="rr-mini-label">Total Webinars</p>
                        </div>
                    </div>
                    ...
                    <div class="rr-mini-stat green">
                        ...
                        <div>
                            <p class="rr-mini-value">{{ $stats['published'] }}</p>
            ...
            {{-- Decorative illustration: a live webinar screen with an audience --}}
        ...
        <div class="rr-tabbar mb-4">
            @foreach ($tabsList as $t)
                <a href="{{ route('mentor.webinars.index', array_filter(['status' => $t['status'], 'q' => $search])) }}"
                   class="rr-tab {{ $activeStatus === $t['status'] ? 'rr-tab-active' : '' }}">
                    {{ $t['label'] }}
        ...
        {{-- ===================== MAIN GRID ===================== --}}
        <div class="rr-row wb-row">

            {{-- ---- LEFT / MAIN COLUMN: Webinars list ---- --}}
            <div class="rr-col-left wb-col-main">

                <div class="card rr-card border-0 mb-4">
                    <div class="card-header bg-white border-0 pb-3 pt-3 px-3">
                        <h2 class="h6 fw-semibold mb-3">Webinars / Workshops List</h2>
                        <form method="GET" action="{{ route('mentor.webinars.index') }}">
                            @if($activeStatus)
                    ...
                    <div class="list-group list-group-flush">
                        ...
                        @forelse ($webinars as $index => $webinar)
                            @php
                                ...
                                $statusInfo = $statusMap[$webinar->status] ?? ['label' => ucfirst($webinar->status), 'class' => 'rr-badge-blue'];
                                $bannerUrl = $webinar->banner ? Storage::url($webinar->banner) : '';
                            @endphp

                            <div class="list-group-item py-3 border-0 border-bottom wb-webinar-item" role="button" tabindex="0"
                                ...
                                <div class="d-flex align-items-start gap-3">
                                    @if($bannerUrl)
                                        <img src="{{ $bannerUrl }}" class="rounded-3 flex-shrink-0 wb-item-thumb" alt="">
                                    @else
                                        <div class="wb-item-thumb wb-item-icon {{ $accent }} flex-shrink-0">
                                            <i class="fa-solid fa-chalkboard-user"></i>
                                        </div>
                                    @endif
                                    ...
                                    <div class="flex-grow-1 min-w-0">
                                        <div class="d-flex justify-content-between align-items-start gap-2">
                                            <p class="fw-semibold mb-0 text-truncate">{{ $webinar->title }}</p>
                                            <span class="rr-time flex-shrink-0">{{ $webinar->scheduled_date->diffForHumans(null, true) }}</span>
                                        </div>
                                        ...
                                        <div class="d-flex flex-wrap gap-1 align-items-center">
                                            @if($webinar->category)
                                                <span class="rr-tag rr-tag-1">{{ strtoupper($webinar->category) }}</span>
                                            @endif
                                            <span class="rr-tag rr-tag-0">{{ strtoupper($webinar->type) }}</span>
                                            <span class="rr-pill {{ $statusInfo['class'] }}">{{ $statusInfo['label'] }}</span>
                                        </div>
                                        <p class="small text-muted mb-0 mt-2">
                                            <i class="fa-regular fa-calendar me-1"></i>{{ $webinar->scheduled_date->format('d M Y') }}
                                            &middot; {{ \Carbon\Carbon::parse($webinar->scheduled_time)->format('h:i A') }}
                                            &middot; <i class="fa-solid fa-users me-1"></i>{{ $webinar->registrations_count }}{{ $webinar->capacity ? '/' . $webinar->capacity : '' }}
                                        </p>
                                    </div>
                                    ...
                                    <div class="row-actions-wrap flex-shrink-0" onclick="event.stopPropagation()">
                                        ...
                                        <div class="row-actions-menu">
                                            <a href="{{ route('mentor.webinars.registrations.index', $webinar) }}" class="row-menu-item">
                                                <i class="fa-solid fa-users"></i> View Registrations
                                            </a>
                                            <a href="{{ route('mentor.webinars.attendance', $webinar) }}" class="row-menu-item">
                                                <i class="fa-solid fa-clipboard-check"></i> Attendance &amp; Resources
                                            </a>
                                            <a href="{{ route('mentor.webinars.edit', $webinar) }}" class="row-menu-item">
                                                <i class="fa-solid fa-pen"></i> Edit
                                            </a>
                                            <form action="{{ route('mentor.webinars.destroy', $webinar) }}" method="POST"
            ...
            <div class="wb-col-side">
                ...
                <div class="card rr-card rr-side-card border-0 mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <a href="{{ route('mentor.webinars.create') }}" class="rr-hero-btn w-100 justify-content-center mb-3">
                            <i class="fa-solid fa-plus"></i> Create Webinar / Workshop
                        </a>

                        <a href="{{ route('mentor.webinars.create') }}?type=webinar" class="wb-quick-action">
                            <span class="wb-qa-icon rr-tag-0"><i class="fa-solid fa-video"></i></span>
                            <span class="wb-qa-text">
                                Host a Webinar
                                <small>Schedule a live online session</small>
                            </span>
                        </a>
                        <a href="{{ route('mentor.webinars.create') }}?type=workshop" class="wb-quick-action">
                            <span class="wb-qa-icon rr-tag-2"><i class="fa-solid fa-chalkboard-user"></i></span>
                            <span class="wb-qa-text">
                                Host a Workshop
                                <small>Plan a hands-on training session</small>
                            </span>
                        </a>
                    </div>
                </div>
                ...
                <div class="card rr-card rr-side-card border-0 mb-4">
                    <div class="rr-side-card-accent"></div>
                    <div class="card-body">
                        <p class="rr-side-label mb-3"><i class="fa-solid fa-chart-simple"></i> Webinar Overview</p>
                        <ul class="list-unstyled small mb-0 rr-side-list wb-overview-list">
                            <li><span class="wb-dot" style="background:#3376F2;"></span>Total Webinars <span class="wb-count">{{ $stats['total'] }}</span></li>
                            <li><span class="wb-dot" style="background:#22B573;"></span>Approved <span class="wb-count">{{ $stats['approved'] }}</span></li>
                            <li><span class="wb-dot" style="background:#F5A623;"></span>Pending Approval <span class="wb-count">{{ $stats['pending'] }}</span></li>
                            <li><span class="wb-dot" style="background:#EF5350;"></span>Rejected <span class="wb-count">{{ $stats['rejected'] }}</span></li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="card rr-card rr-side-card border-0 mb-4">
                    <div class="card-header bg-white d-flex justify-content-between align-items-center py-3 border-0">
                        <h3 class="h6 fw-semibold mb-0">
                            <i class="fa-solid fa-clock-rotate-left me-2" style="color:#3376F2;"></i>Latest Webinars
                        </h3>
                        <a href="{{ route('mentor.webinars.index') }}" class="rr-side-link rr-side-link-sm">View All</a>
                    </div>
                    <div class="list-group list-group-flush">
                        @php
                            $upStatusMap = [
                        ...
                        @forelse ($upcoming as $item)
                            <div class="list-group-item py-3 border-0 border-bottom rr-history-item">
                                <div class="d-flex align-items-center gap-2">
                                    @if($item->banner)
                                        <img src="{{ Storage::url($item->banner) }}" class="rounded-3 flex-shrink-0" width="32" height="32" style="object-fit:cover;" alt="">
                                    @else
                                        <span class="wb-item-icon rr-tag-0 flex-shrink-0" style="width:32px;height:32px;font-size:13px;">
                                            <i class="fa-solid fa-chalkboard-user"></i>
                                        </span>
                                    @endif
                                    <div class="flex-grow-1 min-w-0">
                                        ...
                                        <p class="small text-muted mb-0 text-truncate">
                                            {{ $item->scheduled_date->format('d M Y') }} &middot; {{ $item->platform ?? ucfirst($item->type) }}
                                        </p>
                                    </div>
                                    <span class="rr-pill {{ $upStatusMap[$item->status] ?? 'rr-badge-blue' }} flex-shrink-0">{{ ucfirst($item->status) }}</span>
                                </div>
                            </div>
                        @empty
                            <div class="list-group-item text-center text-muted py-4 small border-0">No upcoming webinars.</div>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="card rr-card rr-side-card border-0">
                    <div class="rr-side-card-accent"></div>
                    <div class="card-body">
                        <p class="rr-side-label mb-3"><i class="fa-regular fa-lightbulb"></i> Tips for a Successful Webinar</p>
                        <ul class="wb-tips-list small text-muted mb-0">
...
<div class="wb-modal-overlay" id="wbModalOverlay" onclick="wbCloseDetailsModal(event)">
    <div class="wb-modal" onclick="event.stopPropagation()">
        <button type="button" class="wb-modal-close" onclick="wbCloseDetailsModal()" aria-label="Close">
            <i class="fa-solid fa-xmark"></i>
        </button>

        <div class="wb-modal-cover" id="wbModalCover" style="display:none;"></div>

        <div class="wb-modal-body">
            <span class="rr-pill mb-3 d-inline-flex" id="wbModalStatus"></span>

            <h2 id="wbModalTitle" class="h5 fw-semibold mb-2"></h2>

            <div class="d-flex flex-wrap gap-2 mb-3">
                <span class="rr-tag rr-tag-1" id="wbModalCategory"></span>
                <span class="rr-tag rr-tag-0" id="wbModalType"></span>
            </div>

            <p class="small text-muted mb-4" id="wbModalDescription"></p>

            <ul class="list-unstyled small mb-4 rr-side-list wb-modal-stats">
                <li><i class="fa-regular fa-calendar"></i><span id="wbModalDate"></span></li>
                <li><i class="fa-regular fa-clock"></i><span id="wbModalTime"></span></li>
                <li><i class="fa-solid fa-users"></i><span id="wbModalRegistrations"></span>&nbsp;Registered</li>
                <li><i class="fa-solid fa-chair"></i><span id="wbModalCapacity"></span>&nbsp;Capacity</li>
            </ul>

            <a href="#" id="wbModalMeetingBtn" class="rr-hero-btn" target="_blank" rel="noopener" style="display:none;">
...
/* =========================================================
   WEBINARS INDEX
   Same design language as the Mentorship Requests and
   Resume Reviews pages: soft blue/purple palette, card
   panels, CSS-drawn hero illustration.
   ========================================================= */
...
.wb-col-main, .wb-col-side { width: 100%; min-width: 0; }
.wb-col-side > .card:not(:last-child) { margin-bottom: 18px !important; }
...
/* =========================================================
   HERO VISUAL - a live webinar screen with an audience
   ========================================================= */
...
.rr-col-left .card-header, .wb-col-main .card-header { padding: 18px 16px !important; }
.rr-col-left .card-header h2, .wb-col-main .card-header h2 { font-size: 17px !important; color: #1F2937; font-weight: 700 !important; }
...
/* ---- Webinar list item ---- */
.wb-webinar-item { padding: 15px 14px !important; border-bottom: 1px solid #F0F2F5 !important; transition: background .15s ease; background: #ffffff; cursor: pointer; }
.wb-webinar-item:hover { background: #F8FAFF; }
.wb-webinar-item .fw-semibold { font-size: 15px; color: #1F2937; }
.wb-webinar-item .small { font-size: 13px; }

.wb-item-thumb { width: 56px; height: 56px; object-fit: cover; }
.wb-item-icon { display: flex; align-items: center; justify-content: center; border-radius: 10px; font-size: 20px; }

.rr-time { background: #FFF6E7; color: #C47A00; font-size: 11px; font-weight: 700; padding: 4px 8px; border-radius: 999px; }
...
.row-actions-menu { display: none; position: absolute; right: 0; top: calc(100% + 6px); background: #fff; border: 1px solid #E7EAF0; border-radius: 10px; box-shadow: var(--rr-shadow); min-width: 200px; z-index: 20; overflow: hidden; }
...
/* Quick actions */
.wb-quick-action { display: flex; align-items: center; gap: 12px; padding: 12px; border-radius: 10px; text-decoration: none; margin-bottom: 8px; transition: background .15s ease; }
.wb-quick-action:last-child { margin-bottom: 0; }
.wb-quick-action:hover { background: #F8FAFF; }
.wb-qa-icon { width: 36px; height: 36px; border-radius: 10px; display: inline-flex; align-items: center; justify-content: center; font-size: 15px; flex-shrink: 0; }
.wb-qa-text { display: flex; flex-direction: column; font-size: 13px; font-weight: 600; color: #1F2937; }
.wb-qa-text small { font-size: 11.5px; font-weight: 400; color: #6B7280; }

/* Overview list */
.wb-overview-list li { display: flex; align-items: center; gap: 10px; color: #374151; font-size: 13px; font-weight: 500; margin-bottom: 12px !important; }
.wb-overview-list li:last-child { margin-bottom: 0 !important; }
.wb-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
.wb-count { margin-left: auto; font-weight: 700; color: #1F2937; }

/* Review/history style rows for latest webinars */
...
/* Tips */
.wb-tips-list { padding-left: 18px; margin: 0; line-height: 1.7; }

/* ---- Pagination ---- */
.wb-col-main .pagination { justify-content: center; margin: 10px 0; }
.wb-col-main .pagination .page-link { font-size: 13px; border-radius: 6px; margin: 0 2px; color: var(--rr-primary); }
.wb-col-main .pagination .active .page-link { background: var(--rr-primary); border-color: var(--rr-primary); color: #ffffff; }
...
.wb-modal-cover { width: 100%; height: 200px; overflow: hidden; }
.wb-modal-body { padding: 24px; }
.wb-modal-stats li { display: flex; align-items: center; gap: 10px; color: #667085; font-size: 13px; margin-bottom: 10px !important; }
.wb-modal-stats i { width: 16px; text-align: center; color: #7B8798; font-size: 12px; flex-shrink: 0; }
...
@media (max-width: 767px) {
    ...
    .wb-webinar-item .fw-semibold { font-size: 14px; }
    .wb-webinar-item .small { font-size: 12px; }
    .wb-item-thumb, .wb-item-icon { width: 44px; height: 44px; }
}
...
document.addEventListener('DOMContentLoaded', function () {
    // ---------- 3-dot row menu ----------
    window.wbToggleRowMenu = function (btn) {
    ...
    // ---------- Webinar details modal ----------
    window.wbOpenDetailsModal = function (card) {
