...
<div class="mentor-shell">
    ...
    <div class="mentor-card"
        style="margin-bottom:16px; border-color:var(--mt-success); background:var(--mt-success-bg); color:var(--mt-success); font-size:14px;">
        {{ session('success') }}
    </div>
    @endif
    ...
    <div class="mentor-hero">
        <div class="mentor-hero-content">
            ...
            <p>
                Create, manage and share your webinars and workshops. Track registrations, approval status and
    ...
    <div class="mentor-layout">
        ...
        <div>
            ...
            <div class="mentor-card" style="padding:22px 22px 8px;">
                ...
                <div class="material-list">
                    ...
                    $statusInfo = $statusMap[$webinar->status] ?? ['label' => ucfirst($webinar->status), 'class' =>
                    'badge-default'];
                    $bannerUrl = $webinar->banner ? Storage::url($webinar->banner) : '';
                    @endphp

                    <div class="material-card" role="button" tabindex="0" onclick="wbOpenDetailsModal(this)"
                        onkeydown="if(event.key==='Enter'){wbOpenDetailsModal(this)}" data-title="{{ $webinar->title }}"
                        data-description="{{ $webinar->description ?: 'No description provided.' }}"
                        data-category="{{ $webinar->category ?: '—' }}" data-type="{{ strtoupper($webinar->type) }}"
                        data-status="{{ $statusInfo['label'] }}" data-status-class="{{ $statusInfo['class'] }}"
                        ...
                        data-capacity="{{ $webinar->capacity ?: '—' }}" data-banner="{{ $bannerUrl }}"
                        data-meeting-link="{{ $webinar->meeting_link ?: '' }}" data-icon="fa-solid fa-chalkboard-user"
                        data-accent="{{ $accent }}">
                        ...
                        <div class="row-actions-wrap" onclick="event.stopPropagation()">
                            <button type="button" class="row-actions-btn" onclick="wbToggleRowMenu(this)"
                                aria-label="Row actions">
                                <i class="fa-solid fa-ellipsis"></i>
                            </button>
                            <div class="row-actions-menu">

                                <a href="{{ route('mentor.webinars.registrations.index', $webinar) }}"
                                    class="row-menu-item">
                                    <i class="fa-solid fa-users"></i>
                                    View Registrations
                                </a>
        ...
        <div>
            <div class="sidebar-card">
                ...
                <a href="{{ route('mentor.webinars.create') }}" class="btn btn-primary"
                ...
                <a href="{{ route('mentor.webinars.create') }}?type=webinar" class="quick-action-btn">
                    <span class="qa-icon" style="background:#EAF1FF;color:#3b6ff8;"><i
                ...
                <a href="{{ route('mentor.webinars.create') }}?type=workshop" class="quick-action-btn">
                    <span class="qa-icon" style="background:#F1EBFE;color:#7C3AED;"><i
            ...
            <div class="sidebar-card">
                ...
                <div class="ranked-row">
                    ...
                    <span class="name" style="flex:1;">
                        ...
                        <small>{{ $item->scheduled_date->format('d M Y') }} &middot;
                            {{ $item->platform ?? ucfirst($item->type) }}</small>
                    </span>
                    <span
                        class="tag-status {{ $upStatusMap[$item->status] ?? 'badge-default' }}">{{ ucfirst($item->status) }}</span>
                </div>
                @empty
            ...
            <div class="sidebar-card">
                <div class="sidebar-card-header">
                    <h3><i class="fa-regular fa-lightbulb"></i> Tips for a Successful Webinar</h3>
                </div>
    ...
    <div class="mt-modal" onclick="event.stopPropagation()">
        ...
        <div class="mt-modal-body">
            ...
            <div class="mt-modal-footer">
                <a href="#" id="wbModalMeetingBtn" class="btn btn-primary" target="_blank" rel="noopener"
                    style="display:none;">
...
function wbOpenDetailsModal(card) {
    ...
    if (d.banner) {
        ...
        coverEl.innerHTML = '<img src="' + d.banner +
