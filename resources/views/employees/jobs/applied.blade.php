    ...
    <section class="max-w-5xl mx-auto px-6 pt-10 pb-24">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <h1 class="font-display font-bold text-2xl text-ink">Applied Jobs</h1>
                <p class="text-sm text-slate2 mt-1">{{ $jobs->total() }} {{ Str::plural('application', $jobs->total()) }} submitted</p>
            </div>
                ...
                <article class="bg-white rounded-2xl shadow-card ring-1 ring-black/[0.03] p-4 sm:p-5 flex gap-4 items-start">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center justify-between mt-3.5">
                            <span class="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-mint/10 text-mint border border-mint/20 inline-flex items-center gap-1">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <form action="{{ route('employee.jobs.save', $job->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" aria-label="{{ $isSaved ? 'Unsave job' : 'Save job' }}"
                                            class="w-8 h-8 flex items-center justify-center rounded-lg transition-colors
                                                   {{ $isSaved ? 'text-brand bg-brand/10' : 'text-slate2 hover:text-brand hover:bg-brand/5' }}
                                                   focus-visible:ring-2 focus-visible:ring-brand">
                                        <svg class="w-4 h-4" fill="{{ $isSaved ? 'currentColor' : 'none' }}" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
            ...
            @empty
                <div class="bg-white rounded-2xl shadow-card p-14 text-center">
                    <div class="w-12 h-12 rounded-full bg-surface flex items-center justify-center mx-auto mb-4">
                        <svg class="w-5 h-5 text-slate2" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                    </div>
                    <p class="font-display font-bold text-sm text-ink">No applications yet</p>
                    <p class="text-xs text-slate2 mt-1">Jobs you apply to will show up here.</p>
