...
{
    /**
     * Articles hub for Mentors — categories, filters, tabs, trending sidebar.
     * Only shows articles an admin has APPROVED. Pending/rejected
     * submissions (including ones submitted by employees) never appear here.
     */
    public function index(Request $request)
    {
        ...
        if (! Schema::hasTable('articles')) {
            return view('mentor.articles.index', [
                'activeTab'       => $tab,
    ...
    /**
     * Single article data — used ONLY by the popup modal on the mentor index page.
     * Always returns JSON. There is no separate show.blade.php.
     * Note: works on the same `articles` table employees submit to — a mentor
     * only ever sees articles that have already been approved by an admin,
     * enforced by the `approved()` scope used everywhere above.
     */
   public function show(Request $request, Article $article)
{
    // Only approved articles can be viewed
    if (
        !method_exists($article, 'isApproved') &&
    ...
    return response()->json([
        ...
        'comments' => $article->comments->map(function ($c) {
            return [
                'id'         => $c->id,
